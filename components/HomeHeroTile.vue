...
const getGlobeWrapperStyle = (n) => {
  ...
  // Base 3D curved globe layout depth
  const baseZ = (xNorm + yNorm) * 40
  
  // Continuous wave offset calculation using timeElapsed for smooth undulation
  const waveOffset = Math.sin(timeElapsed.value + col * 0.6 + row * 0.5) * 18

  return {
    transform: `translateZ(${baseZ + waveOffset}px)`,
...
onMounted(() => {
  ...
  // Continuous wave loop ticker
  waveAnimation = gsap.ticker.add(() => {
    timeElapsed.value += 0.025
  })
...
onUnmounted(() => {
  if (ctx) ctx.revert()
  if (waveAnimation) gsap.ticker.remove(waveAnimation)
})
...
<template>
  <div class="min-h-screen selection:bg-secondary/25 selection:text-white font-sans antialiased bg-slate-950">
    ...
    <!-- HERO HEADER WITH 3D CURVED GLOBE GRID -->
    <section 
      ref="containerRef"
      ...
      <ClientOnly>
        <div class="absolute inset-0 pointer-events-none overflow-hidden z-1">
          <div 
            ...
            :key="col.id"
            class="absolute top-[-100%] font-mono text-[30px] sm:text-lg text-sky-600/80 flex flex-col items-center select-none animate-binary-fall"
            :style="{
              left: `${col.left}%`,
      ...
      <div class="absolute inset-0 pointer-events-none flex items-center justify-center overflow-visible z-20" style="transform-style: preserve-3d;">
        ...
        >
          <div 
            ...
            class="relative flex items-center justify-center pointer-events-none transition-transform duration-75 ease-out"
          >
            ...
            <div class="absolute inset-[-12px] pointer-events-auto flex items-center justify-center cursor-pointer group">
              <div 
                class="glass-tile w-full h-full relative rounded-2xl sm:rounded-[2rem] bg-slate-900/90 backdrop-blur-xl overflow-hidden flex items-center justify-center p-3 transform-gpu min-h-[110px] sm:min-h-[auto]"
                style="transform-style: preserve-3d;"
              >
                <!-- OPTIMIZED PROJECT IMAGES WITH NUXTIMG -->
                <template v-if="n === 2">
                  <NuxtImg src="/images/casatechllc-og-image.png" alt="Roof Estimator Flyer" format="webp" quality="80" class="absolute inset-0 w-full h-full object-cover opacity-85 group-hover:opacity-100 group-hover:scale-105 transition-all duration-300 pointer-events-none" />
                ...
                <template v-else-if="n === 6">
                  <NuxtImg src="/images/project-casatech-roofestimator-estimate.png" alt="Roof Estimator Quoting Engine" format="webp" quality="80" class="absolute inset-0 w-full h-full object-cover opacity-85 group-hover:opacity-100 group-hover:scale-105 transition-all duration-300 pointer-events-none" />
                  <div class="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-transparent to-transparent pointer-events-none"></div>
                </template>
                <template v-else-if="n === 4">
                  <NuxtImg src="/images/project-casatech-roofestimator-tookbox.png" alt="Roof Estimator Toolkit" format="webp" quality="80" class="absolute inset-0 w-full h-full object-cover opacity-85 group-hover:opacity-100 group-hover:scale-105 transition-all duration-300 pointer-events-none" />
                  <div class="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-transparent to-transparent pointer-events-none"></div>
                </template>
                <template v-else-if="n === 5">
                  <NuxtImg src="/images/project-nvchva-mockup-4by3.jpg" alt="NCHVA Mockup Layout" format="webp" quality="80" class="absolute inset-0 w-full h-full object-cover opacity-85 group-hover:opacity-100 group-hover:scale-105 transition-all duration-300 pointer-events-none" />
                  <div class="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-transparent to-transparent pointer-events-none"></div>
                </template>
                <template v-else-if="n === 7">
                  <NuxtImg src="/images/project-nchva-graph.png" alt="NCHVA Graph Data" format="webp" quality="80" class="absolute inset-0 w-full h-full object-cover opacity-85 group-hover:opacity-100 group-hover:scale-105 transition-all duration-300 pointer-events-none" />
                  <div class="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-transparent to-transparent pointer-events-none"></div>
                </template>
                ...
                <template v-else-if="n === 11">
                  <img src="/images/project-casatech-roofestimator-flyer.jpg" alt="Casatech OG Blueprint" class="absolute inset-0 w-full h-full object-cover opacity-85 group-hover:opacity-100 group-hover:scale-105 transition-all duration-300 pointer-events-none" />
                  <div class="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-transparent to-transparent pointer-events-none"></div>
                </template>
                ...
                <template v-else-if="n === 14">
                  <NuxtImg src="/images/project-omarsbarbershop-barberscarousel.png" alt="Omar's Barbershop Carousel" format="webp" quality="80" class="absolute inset-0 w-full h-full object-cover opacity-85 group-hover:opacity-100 group-hover:scale-105 transition-all duration-300 pointer-events-none" />
                  <div class="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-transparent to-transparent pointer-events-none"></div>
                </template>
                ...
                <template v-else-if="n === 18">
                  <NuxtImg src="/images/project-omarsbarbershop-suites.png" alt="Omar's Barbershop Suites" format="webp" quality="80" class="absolute inset-0 w-full h-full object-cover opacity-85 group-hover:opacity-100 group-hover:scale-105 transition-all duration-300 pointer-events-none" />
                  <div class="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-transparent to-transparent pointer-events-none"></div>
                </template>
                ...
                <template v-else-if="n === 21">
                  <NuxtImg src="/images/project-casatech-phone.jpg" alt="Casatech Mobile Responsive Preview" format="webp" quality="80" class="absolute inset-0 w-full h-full object-cover opacity-85 group-hover:opacity-100 group-hover:scale-105 transition-all duration-300 pointer-events-none" />
                  <div class="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-transparent to-transparent pointer-events-none"></div>
                </template>
                <template v-else-if="n === 23">
      ...
      <ClientOnly>
        <div class="absolute inset-0 pointer-events-none overflow-hidden z-25">
          <div 
            ...
            :key="col.id"
            class="absolute top-[-100%] font-mono text-sm sm:text-md text-sky-400 flex flex-col items-center select-none animate-binary-fall"
            :style="{
              left: `${col.left}%`,
              animationDuration: `${col.duration}s`,
              animationDelay: `${col.delay}s`,
...
/* 3D Extruded Gradient Border Effect (Shiny Sky Blue to Silver/Grey) */
.glass-tile {
  border: 2px solid transparent;
  background-origin: border-box;
  background-clip: padding-box, border-box;
  background-image: linear-gradient(rgba(15, 23, 42, 0.9), rgba(15, 23, 42, 0.9)), 
                    linear-gradient(135deg, rgba(56, 189, 248, 0.9) 0%, rgba(148, 163, 184, 0.7) 50%, rgba(203, 213, 225, 0.8) 100%);
  box-shadow: 0 15px 35px -5px rgba(0, 0, 0, 0.8), 
              inset 0 1px 0 0 rgba(56, 189, 248, 0.4),
              inset 0 -1px 0 0 rgba(148, 163, 184, 0.3);
...
.group:hover .glass-tile {
  ...
  box-shadow: 0 40px 80px 0 rgba(0, 119, 182, 0.5),
              inset 0 2px 4px 0 rgba(56, 189, 248, 0.8);
  z-index: 50;
}
</style>